...
const OrganizationSettings = ({ organization }) => {
  ...
  return (
    <div>
      ...
      <AccountManagementSettings organization={organization} />
    </div>
  )
}

// Account Management Settings (Expiring Soon Threshold)
const AccountManagementSettings = ({ organization }) => {
  const queryClient = useQueryClient()
  const [threshold, setThreshold] = useState(
    organization?.settings?.expiring_soon_days || 30
  )
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await organizationsAPI.updateSettings({
        ...currentSettings,
        expiring_soon_days: parseInt(threshold) || 30
      })
      queryClient.invalidateQueries(['organization'])
      queryClient.invalidateQueries(['accounts'])
      toast.success('Account management settings saved')
    } catch (error) {
  ...
  return (
    <div className="border-t border-gray-200 mt-8 pt-8">
      <h4 className="text-base font-semibold text-gray-900 mb-4">Account Management</h4>
      <div className="max-w-md">
        ...
        <p className="text-xs text-gray-500 mt-2">
          Number of days before renewal date to consider an account as &ldquo;expiring soon&rdquo;. Used in filters, stats cards, and exports.
        </p>
        <div className="mt-4">
          <button
            onClick={handleSave}

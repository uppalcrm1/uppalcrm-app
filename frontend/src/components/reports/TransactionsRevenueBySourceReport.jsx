 ...
 * Standard pre-built report showing revenue grouped by source for a selected month
 */
const TransactionsRevenueBySourceReport = () => {
  const now = new Date();
  const [selectedYear, setSelectedYear] = useState(now.getFullYear());
  const [selectedMonth, setSelectedMonth] = useState(now.getMonth() + 1);
  const [chartType, setChartType] = useState('table'); // 'table', 'bar', 'pie'

  // Fetch transactions revenue by source
  const { data: reportData, isLoading, error, refetch } = useQuery({
    queryKey: ['transactionsRevenueBySource', { year: selectedYear, month: selectedMonth }],
    queryFn: () => reportingAPI.getTransactionRevenueBySource(selectedYear, selectedMonth)
  });
  ...
  const handleExportCSV = () => {
    ...
    // Create CSV content
    const headers = ['Source', 'Transaction Count', 'Revenue', 'Percentage'];
    ...
    rows.push([]);
    rows.push(['SUMMARY', '', '', '']);
    rows.push(['Total Transactions', summary.totalTransactions, `$${summary.totalAmount.toFixed(2)}`, '100%']);
    rows.push(['Top Source (by Revenue)', summary.topSource, `$${summary.topSourceAmount.toFixed(2)}`, `${((summary.topSourceAmount / summary.totalAmount) * 100).toFixed(2)}%`]);
    ...
    link.setAttribute('download', `transactions-revenue-by-source-${timestamp}.csv`);
    link.click();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Transactions Revenue by Source</h2>
          <p className="text-gray-600 text-sm mt-1">View revenue volume grouped by source</p>
        </div>
      </div>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              ...
              <p className="text-3xl font-bold text-gray-900">${reportData.data.summary.totalAmount.toFixed(2)}</p>
              <p className="text-xs text-gray-500 mt-2">in {monthDisplay}</p>
            </div>

            {/* Top Source by Revenue */}
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <p className="text-gray-600 text-sm font-medium mb-2">Top Source (by Revenue)</p>
              <p className="text-2xl font-bold text-gray-900">{reportData.data.summary.topSource}</p>
              <p className="text-xs text-gray-500 mt-2">${reportData.data.summary.topSourceAmount.toFixed(2)}</p>
            </div>

            {/* Top Source Percentage */}
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <p className="text-gray-600 text-sm font-medium mb-2">Top Source % of Revenue</p>
              <p className="text-3xl font-bold text-green-600">
                {reportData.data.summary.totalAmount > 0
                  ? ((reportData.data.summary.topSourceAmount / reportData.data.summary.totalAmount) * 100).toFixed(1)
                  : 0}%
              </p>
...
export default TransactionsRevenueBySourceReport;

 ...
 * Standard pre-built report showing transaction revenue grouped by lead owner at time of conversion
 */
const TransactionsRevenueByOwnerReport = () => {
  const now = new Date();
  const [selectedYear, setSelectedYear] = useState(now.getFullYear());
  const [selectedMonth, setSelectedMonth] = useState(now.getMonth() + 1);
  const [chartType, setChartType] = useState('table'); // 'table', 'bar', 'pie'

  // Fetch transaction revenue by owner
  const { data: reportData, isLoading, error, refetch } = useQuery({
    queryKey: ['transactionsRevenueByOwner', { year: selectedYear, month: selectedMonth }],
    queryFn: () => reportingAPI.getTransactionRevenueByOwner(selectedYear, selectedMonth)
  });
  ...
  const handleExportCSV = () => {
    ...
    // Create CSV content
    const headers = ['Owner Name', 'Transaction Count', 'Revenue', 'Percentage'];
    ...
    rows.push(['Top Owner (by Revenue)', summary.topOwner, '', `${summary.topOwnerPercentage}%`]);
    ...
    link.setAttribute('download', `transaction-revenue-by-owner-${timestamp}.csv`);
    link.click();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Transaction Revenue by Owner</h2>
          <p className="text-gray-600 text-sm mt-1">View revenue by lead owner at time of conversion</p>
        </div>
      </div>
      ...
      {!isLoading && reportData?.data && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {/* Top Owner by Revenue */}
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <p className="text-gray-600 text-sm font-medium mb-2">Top Owner (by Revenue)</p>
              <p className="text-2xl font-bold text-gray-900">{reportData.data.summary.topOwner}</p>
              <p className="text-xs text-gray-500 mt-2">${reportData.data.summary.topOwnerAmount.toFixed(2)}</p>
            </div>

            {/* Top Owner Revenue Percentage */}
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <p className="text-gray-600 text-sm font-medium mb-2">Top Owner % of Revenue</p>
...
export default TransactionsRevenueByOwnerReport;

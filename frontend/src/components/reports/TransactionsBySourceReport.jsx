 ...
 * Standard pre-built report showing transaction count grouped by source for a selected month
 */
const TransactionsBySourceReport = () => {
  const now = new Date();
  const [selectedYear, setSelectedYear] = useState(now.getFullYear());
  const [selectedMonth, setSelectedMonth] = useState(now.getMonth() + 1);
  const [chartType, setChartType] = useState('table'); // 'table', 'bar', 'pie'

  // Fetch transactions by source
  const { data: reportData, isLoading, error, refetch } = useQuery({
    queryKey: ['transactionsBySource', { year: selectedYear, month: selectedMonth }],
    queryFn: () => reportingAPI.getTransactionsBySource(selectedYear, selectedMonth)
  });
  ...
  const handleExportCSV = () => {
    ...
    // Create CSV content
    const headers = ['Source', 'Transaction Count', 'Percentage', 'Total Amount'];
    ...
    rows.push(['Top Source', summary.topSource, `${((summary.topSourceCount / summary.totalTransactions) * 100).toFixed(2)}%`, '']);
    ...
    link.setAttribute('download', `transactions-by-source-${timestamp}.csv`);
    link.click();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Transactions by Source</h2>
          <p className="text-gray-600 text-sm mt-1">View transaction volume grouped by source</p>
        </div>
      </div>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              ...
              <p className="text-xs text-gray-500 mt-2">{reportData.data.summary.topSourceCount} transactions</p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              ...
              <p className="text-3xl font-bold text-blue-600">
                {reportData.data.summary.totalTransactions > 0
                  ? ((reportData.data.summary.topSourceCount / reportData.data.summary.totalTransactions) * 100).toFixed(1)
                  : 0}%
              </p>
...
export default TransactionsBySourceReport;

-- =============================================================================
-- SAFE FIX-ONLY MIGRATION SCRIPT
...
-- =============================================================================
-- 3. RECREATE INDEXES FOR CONTACTS (safely)
-- =============================================================================
...
DROP INDEX IF EXISTS idx_contacts_last_contact_date;
CREATE INDEX idx_contacts_last_contact_date ON contacts(last_contact_date);

DROP INDEX IF EXISTS idx_contacts_next_follow_up;
...
-- =============================================================================
-- 6. CREATE CONVENIENCE VIEWS FOR API COMPATIBILITY
-- =============================================================================

-- Create a view that provides the simplified contact interface expected by the API
DROP VIEW IF EXISTS contacts_api;
CREATE VIEW contacts_api AS
SELECT 
    id,
    tenant_id,
    name,
...
BEGIN
    ...
    BEGIN
        ...
        SELECT * INTO NEW FROM contacts_api WHERE email = NEW.email AND organization_id = COALESCE(NEW.tenant_id, NEW.organization_id) LIMIT 1;
        RETURN NEW;
    END;
END;
$$ LANGUAGE plpgsql;
...
SELECT 
    'CONTACTS_TABLE_VERIFICATION' as check_type,
...
-- Check computed columns work with sample data
SELECT 
    'COMPUTED_COLUMNS_TEST' as check_type,
    id,
    first_name,
    last_name,
    name,
...
SELECT 'Safe Contact Management Migration Fix completed successfully!' AS status;